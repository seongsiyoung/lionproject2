...
CREATE TABLE `settlements` (
  ...
  `total_payment_amount` INT NOT NULL COMMENT 'PAYMENT 원장 결제금액 합계',
  `platform_fee` INT NOT NULL COMMENT '플랫폼 수수료',
  `settlement_amount` INT NOT NULL COMMENT '순정산액',
  `refund_amount` INT NOT NULL COMMENT 'REFUND 원장 절대값 합',
  `previous_carry_over_amount` INT NOT NULL COMMENT '이전 누적 미소진 이월액',
  `payable_amount` INT NOT NULL COMMENT '실제 지급액',
  `carry_over_amount` INT NOT NULL COMMENT '다음 달 이월 차감액',
...
CREATE TABLE `settlement_details` (
  ...
  `payment_amount` INT NOT NULL COMMENT '원장 결제 금액',
  `platform_fee` INT NOT NULL COMMENT '플랫폼 수수료',
  `settlement_amount` INT NOT NULL COMMENT '정산 금액',
  `occurred_at` TIMESTAMP NOT NULL COMMENT '원장 발생 시각',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

  UNIQUE KEY `uk_settlement_detail_payment_type` (`payment_id`, `type`),
  ...
  INDEX `idx_settlement_details_occurred` (`occurred_at`),
...
-- ============================================================
-- 17. settlement_targets (정산 배치 대상 스냅샷)
-- ============================================================
CREATE TABLE `settlement_targets` (
  `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
  `mentor_id` BIGINT NOT NULL,
  `settlement_period` VARCHAR(7) NOT NULL COMMENT '정산 기간 (YYYY-MM)',
  `job_instance_id` BIGINT NOT NULL COMMENT 'Spring Batch JobInstance ID',
  `status` VARCHAR(20) NOT NULL DEFAULT 'READY' COMMENT 'READY, PROCESSING, DONE, FAILED, SKIPPED',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

  UNIQUE KEY `uk_settlement_target_job_mentor_period` (`job_instance_id`, `mentor_id`, `settlement_period`),
  INDEX `idx_settlement_targets_job_status` (`job_instance_id`, `status`),
  INDEX `idx_settlement_targets_period` (`settlement_period`),
  INDEX `idx_settlement_targets_mentor` (`mentor_id`),
  CONSTRAINT `fk_settlement_target_mentor` FOREIGN KEY (`mentor_id`) REFERENCES `mentors` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
